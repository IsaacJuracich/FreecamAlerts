...
client.on('ready', async () => {
  console.log(
    chalk.hex(`3D42FE`).bold('[DISCORD]') +
      `\n ${chalk.blueBright.underline(
        `Discord Bot Loaded | ${client.user.tag}`
      )} \n  ${chalk.bold(
        `${client.guilds.cache
          .array()
          .filter(gd => gd.name)
  ...
  if (entity.type == 'player') {
    ...
    try {
      if (cache.anti_freecam.holding_items.includes(entity.heldItem.type)) {
        ...
        let alert_messageFormat =
          '[Anti-Freecam]: [player] Is holding a [item] @ [location]';
        send(
          `${alert_messageFormat
            .toString()
            .replace('[player]', entity.username)
            .replace('[item]', `${getItem(entity.heldItem.type)}`)
            .replace('[location]', entity.position)}`
        );
        client.channels.cache.get(
          client.channels.cache
            .get(`${cache.anti_freecam.alert_channelID}`)
            .send(
              embed.setDescription(
                `**${entity.username}** Is holding a: **${getItem(
                  entity.heldItem.type
...
bot.client.on('itemDrop', async entity => {
  entity.metadata.forEach(function (item) {
    if (item.blockId == 325) {
      ...
      send(
        `[Anti-Freecam]: Bucket Dispensed @ ${string2} | DISP Loc: ${string}`
      );
      client.channels.cache.get(
        client.channels.cache
          .get(`${cache.anti_freecam.alert_channelID}`)
